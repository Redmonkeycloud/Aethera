...
class EmissionCalculator:
    ...
    def _project_construction(self, land_cover_summary: List[Dict], project_capacity_mw: float) -> float:
        area_total = sum(float(record.get("total_area_ha", 0)) for record in land_cover_summary)
        land_clearing = self.factors.project.get("land_clearing_tco2e_per_ha", 0.0) * area_total
        construction = self.factors.project.get("construction_tco2e_per_mw", 0.0) * project_capacity_mw
    ...
    def compute(self, land_cover_summary: List[Dict], project_config: Dict) -> EmissionResult:
        capacity = float(project_config.get("capacity_mw") or self.factors.defaults.get("project_capacity_mw", 0))
