"""Training pipeline for Biodiversity AI model."""
...
from ..models.biodiversity import BiodiversityConfig, BiodiversityEnsemble, CLASS_LABELS
...
def load_training_data(data_path: Path) -> tuple[np.ndarray, np.ndarray]:
    ...
    # Find label column
    label_column = None
    for candidate in ("label", "sensitivity", "target", "biodiversity_sensitivity_class"):
...
def train_biodiversity(
    ...
    """Train Biodiversity AI ensemble model."""
    config = TrainingConfig(
        model_name="biodiversity",
        model_version="0.1.0",
        training_data_path=training_data_path,
        output_dir=Path(output_dir) if output_dir else Path("models/biodiversity"),
    ...
    if training_data_path and Path(training_data_path).exists():
        ...
        ensemble = BiodiversityEnsemble(BiodiversityConfig())
    ...
    model_configs = {
        ...
        "random_forest": {"n_estimators": 200, "max_depth": 8, "random_state": 7},
        "gradient_boosting": {"max_depth": 3, "random_state": 21},
    }
    ...
    for model_name, model_params in model_configs.items():
        ...
        val_metrics = trainer.evaluate_classification(y_val, y_val_pred, labels=CLASS_LABELS, prefix=f"{model_name}_val_")
    ...
    test_metrics = trainer.evaluate_classification(y_test, ensemble_pred, labels=CLASS_LABELS, prefix="ensemble_test_")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train Biodiversity AI model")
