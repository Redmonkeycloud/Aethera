...
# ✅ Import required OSM utils at module level
from core.osm_utils import (
    ensure_roads_gpkg,
    load_roads_layer,
    ensure_poi_gpkg,
    load_poi_layer,
...
# ✅ Import emissions utilities at module level
...
def run_aoi_analysis(country_code, logger):
    ...
    Full AOI analysis:
      ...
      4b) Proximity to Roads, Rivers, Settlements, Ports, Airports
      5) Save outputs (Excel + PNG maps)
    """
    ...
    try:
        ...
        # Optional: save clipped polygons
        paa.save_clipped_as_geojson(
            clipped_pa, os.path.join(OUTPUT_DIR, f"clipped_{pa_source}_{country_code}.geojson")
        )
    ...
    # (4b) Proximity — Roads
    roads_summary_df = pd.DataFrame()
    roads_overlap_df = pd.DataFrame()
    try:
        log_step(logger, "[PROXIMITY] Ensuring cached roads layer via OSMnx...")
        roads_gpkg = ensure_roads_gpkg(
            country_code=country_code, aoi_gdf=aoi_gdf, logger=logger,
            data_dir=DATA_DIR, preferred="osmnx", force=False, buffer_m=5000
        )
        roads_gdf = load_roads_layer(roads_gpkg, logger=logger)

        log_step(logger, "[PROXIMITY] Computing nearest roads...")
        roads_nearest = buffer_analysis.nearest_distance_table(
            aoi_gdf, roads_gdf, label="roads", k=1, return_geometry=True
        )
        roads_summary_df = buffer_analysis.summarize_distance_stats(roads_nearest, label="roads")
        roads_buffers = buffer_analysis.buffer_features(roads_gdf, [250, 500, 1000], dissolve=True)
        roads_overlap_df = buffer_analysis.intersect_aoi_with_buffers(aoi_gdf, roads_buffers)

        buffer_analysis.plot_proximity_map(
            aoi_gdf=aoi_gdf, features_gdf=roads_gdf,
            nearest_lines_gdf=roads_nearest, buffers=roads_buffers,
            outfile=os.path.join(OUTPUT_DIR, f"proximity_roads_{country_code}.png"),
            title=f"AOI → Nearest Roads ({country_code})", logger=logger
        )
        roads_nearest.to_file(os.path.join(OUTPUT_DIR, f"aoi_nearest_roads_{country_code}.geojson"), driver="GeoJSON")
    except Exception as e:
        logger.warning(f"[PROXIMITY] Roads analysis skipped: {e}")

    # (4c) Proximity — Rivers (HydroRIVERS)
    rivers_summary_df = pd.DataFrame()
    rivers_overlap_df = pd.DataFrame()
    try:
        hydro_eu = os.path.join(DATA_DIR, "rivers", "HydroRIVERS_v10_eu_shp", "HydroRIVERS_v10_eu.shp")
        hydro_global = os.path.join(DATA_DIR, "rivers", "HydroRIVERS_v10_shp", "HydroRIVERS_v10.shp")
        hydro_path = hydro_eu if os.path.exists(hydro_eu) else hydro_global

        if os.path.exists(hydro_path):
            log_step(logger, "[PROXIMITY] Loading HydroRIVERS & clipping to AOI bounds...")
            rivers_gdf = buffer_analysis.load_rivers(hydro_path, aoi_gdf=aoi_gdf, logger=logger)
            rivers_nearest = buffer_analysis.nearest_distance_table(
                aoi_gdf, rivers_gdf, label="rivers", k=1, return_geometry=True
            )
            rivers_summary_df = buffer_analysis.summarize_distance_stats(rivers_nearest, label="rivers")
            rivers_buffers = buffer_analysis.buffer_features(rivers_gdf, [250, 500, 1000], dissolve=True)
            rivers_overlap_df = buffer_analysis.intersect_aoi_with_buffers(aoi_gdf, rivers_buffers)

            buffer_analysis.plot_proximity_map(
                aoi_gdf=aoi_gdf, features_gdf=rivers_gdf,
                nearest_lines_gdf=rivers_nearest, buffers=rivers_buffers,
                outfile=os.path.join(OUTPUT_DIR, f"proximity_rivers_{country_code}.png"),
                title=f"AOI → Nearest Rivers ({country_code})", logger=logger
            )
            rivers_nearest.to_file(os.path.join(OUTPUT_DIR, f"aoi_nearest_rivers_{country_code}.geojson"), driver="GeoJSON")
        else:
            logger.warning("[PROXIMITY] HydroRIVERS not found — skipping rivers proximity.")
    except Exception as e:
        logger.warning(f"[PROXIMITY] Rivers analysis skipped: {e}")

    # (4d) Proximity — Settlements / Ports / Airports (OSMnx)
    poi_summaries = {}
    poi_specs = [
        {"type": "settlements", "label": "settlements"},
        {"type": "ports", "label": "ports"},
        {"type": "airports", "label": "airports"},
    ]
    for spec in poi_specs:
        poi_type = spec["type"]
        label = spec["label"]
        try:
            log_step(logger, f"[PROXIMITY] Ensuring cached {poi_type} via OSMnx...")
            gpkg = ensure_poi_gpkg(poi_type, country_code, aoi_gdf, logger=logger, data_dir=DATA_DIR, force=False)
            poi_gdf = load_poi_layer(gpkg, layer=poi_type, logger=logger)

            if poi_gdf.empty:
                logger.warning(f"[PROXIMITY] No {poi_type} features found. Skipping.")
                continue

            nearest = buffer_analysis.nearest_distance_table(
                aoi_gdf, poi_gdf, label=label, k=1, return_geometry=True
            )
            summary = buffer_analysis.summarize_distance_stats(nearest, label=label)
            buffers = buffer_analysis.buffer_features(poi_gdf, [500, 1000, 2000], dissolve=True)
            overlap_df = buffer_analysis.intersect_aoi_with_buffers(aoi_gdf, buffers)

            buffer_analysis.plot_proximity_map(
                aoi_gdf=aoi_gdf,
                features_gdf=poi_gdf,
                nearest_lines_gdf=nearest,
                buffers=buffers,
                outfile=os.path.join(OUTPUT_DIR, f"proximity_{label}_{country_code}.png"),
                title=f"AOI → Nearest {label.capitalize()} ({country_code})",
                logger=logger
            )
            nearest.to_file(os.path.join(OUTPUT_DIR, f"aoi_nearest_{label}_{country_code}.geojson"), driver="GeoJSON")

            poi_summaries[label] = {"summary": summary, "overlap": overlap_df}
        except Exception as e:
            logger.warning(f"[PROXIMITY] {poi_type} analysis skipped: {e}")
    ...
    with pd.ExcelWriter(output_xlsx, mode="w") as writer:
        ...
        if not roads_summary_df.empty:
            roads_summary_df.to_excel(writer, sheet_name="Proximity_Roads_Summary", index=False)
        if not rivers_summary_df.empty:
            rivers_summary_df.to_excel(writer, sheet_name="Proximity_Rivers_Summary", index=False)
        if 'settlements' in poi_summaries:
            poi_summaries['settlements']['summary'].to_excel(writer, sheet_name="Proximity_Settlements", index=False)
            poi_summaries['settlements']['overlap'].to_excel(writer, sheet_name="Buffers_Settlements", index=False)
        if 'ports' in poi_summaries:
            poi_summaries['ports']['summary'].to_excel(writer, sheet_name="Proximity_Ports", index=False)
            poi_summaries['ports']['overlap'].to_excel(writer, sheet_name="Buffers_Ports", index=False)
        if 'airports' in poi_summaries:
            poi_summaries['airports']['summary'].to_excel(writer, sheet_name="Proximity_Airports", index=False)
            poi_summaries['airports']['overlap'].to_excel(writer, sheet_name="Buffers_Airports", index=False)
